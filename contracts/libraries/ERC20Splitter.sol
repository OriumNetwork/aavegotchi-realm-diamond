...
contract ERC20Splitter is ReentrancyGuard {
  ...
  function deposit(
    ...
    require(tokenAddresses.length == shares.length && tokenAddresses.length == recipients.length, "ERC20Splitter: Mismatched input sizes");
    ...
    for (uint256 i = 0; i < userTokens.length; i++) {
      ...
      if (amount > 0) {
        ...
        if (tokenAddress == address(0)) {
          ...
          require(IERC20(tokenAddress).approve(address(this), amount), "ERC20Splitter: Approve failed");

          require(IERC20(tokenAddress).transferFrom(address(this), msg.sender, amount), "ERC20Splitter: TransferFrom failed");
        }
  ...
  function _splitTokens(address tokenAddress, uint256 amount, uint256[] calldata shares, address[] calldata recipients) internal {
    ...
    if (tokenAddress != address(0)) {
      require(IERC20(tokenAddress).transferFrom(msg.sender, address(this), amount), "ERC20Splitter: Transfer failed");
    }
